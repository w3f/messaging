...
At the same time, anonymity systems do assess routers for some combination of perfromance, capacity, and reliability, but do so using centralised infrastructure. 
...
An incentives variant funds rewards with inflation of the rewards token, so users need not pay.
...
In Loopix, mix nodes send cover traffic messages that traverse each strata, eventually looping back to the first strata, and continuing back to their sending mix.  
...
In terms of anonymity, we impact the defenses provided by the loop cover traffic, as well as protections afforded by total traffic.  We’d expect active defenses might be fully utilised before the lottery, but warn that if mixes sample traffic differently, using say Brahms, then this represents a leakage.  We expect minimal reduction in total traffic.  We could investigate using cover traffic originating from users as well, but this requires further analysis, and limiting eligible users remains tricky.
...
An incentivising variant should therefore reward packet senders even
for dropped packets. 
