...
%
We propose a decentralised but probabilistic sampling technique for assessment and/or incentives of mix relay availability and reliability.  
...
If all do so, they are all rewarded in an incentives scheme.  In the end, the more messages a mix is relaying the more chances it has to be rewarded, again subject to the route selection rules for loop cover packets.
...
We assess relays primarily to provide useful information about mixes
to clients, especially each mixes' chances for dropping packets.

We do learn about dropped packets provided mixes publish any winning
$p_{V,i,j}$ even when the cover packet was dropped.  If we worry mixes
might not report dropped packets, then we could use cover loops that
terminate at a random node in the sender's strata, not at the sender
themselves, which do not detect $n-1$ attacks but do hide mix fullness.

In such cases, we could model epochs with a binomial, or perhaps
hypergeometric, distribution for the number of dropped packets out of
the number of packets passing through the node, and thus compute the
assessment using the maximum likelihood estimator over many epochs.
...
We accept the inequitable samples produced by this prodecure, 
...
Intuitively. if stakes nodes are honest then our sampling procedure should provide 

Intuitively, our sampling procedure provides exactly the statistics 

We must produce useful information about nodes  
